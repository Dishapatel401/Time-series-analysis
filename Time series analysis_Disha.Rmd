...
```{r}
squared_residuals <- residuals(cipla_ret_garch1)^2
plot(squared_residuals)

```

```{r}
arch_lm_test_squared <- ArchTest(squared_residuals, lags = 10)
arch_lm_test_squared
```
...
```{r}
squared_residuals2 <- residuals(cipla_ret_garch2)^2
plot(squared_residuals2)
```

```{r}
arch_lm_test_squared2 <- ArchTest(squared_residuals2, lags = 10)
arch_lm_test_squared2
```
